...
let rec evalExp (e : UntypedExp, vtab : VarTable, ftab : FunTable) : Value =
  ...
  | And (e1, e2, pos) ->
        ...
        match (res1, res2) with
          ...
          | _ -> invalidOperands "And on non-bool args: " [(Int, Int)] res1 res2 pos
  | Or (e1, e2, pos) ->
        let res1   = evalExp(e1, vtab, ftab)
        let res2   = evalExp(e2, vtab, ftab)
        match (res1, res2) with
          | (BoolVal n1, BoolVal n2) -> BoolVal (n1 || n2)
          | _ -> invalidOperands "Or on non-bool args: " [(Int, Int)] res1 res2 pos
  | Not(e1, pos) ->
        let res1   = evalExp(e1, vtab, ftab)
        match (res1) with
  ...
  | Negate(e1, pos) ->
        ...
        match (res1) with
          | (IntVal n1) -> IntVal (-n1)
          | _ -> invalidOperand "Negate on non-integer args: " (Int) res1 pos
  | Equal(e1, e2, pos) ->
