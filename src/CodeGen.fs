...
let rec compileExp  (e      : TypedExp)
  ...
  *)
  ...
  | And (e1, e2, pos) ->
      ...
      let code1 = compileExp e1 vtable t1
      let code2 = compileExp e2 vtable t2
      let short_circuit_label = newLab "and_SC"
      code1 @ 
       [ Mips.LI (place, 0)
       ; Mips.BEQ (t1, place, short_circuit_label)
       ] @ code2 @ 
       [ Mips.BEQ (t2, place, short_circuit_label)
       ; Mips.LI (place, 1)
       ; Mips.LABEL short_circuit_label
       ]
  ...
  | Or (e1, e2, pos) ->
      ...
      let code1 = compileExp e1 vtable t1
      let code2 = compileExp e2 vtable t2
      let short_circuit_label = newLab "or_SC"
      code1 @ 
       [ Mips.LI  (place, 1)
       ; Mips.BEQ (t1, place, short_circuit_label)
       ] @ code2 @ 
